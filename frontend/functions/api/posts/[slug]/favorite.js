...
export async function onRequest(context) {
  ...
  const post = await env.DB.prepare(
    'SELECT id, author_id, published, visibility, favorite_count FROM posts WHERE slug = ?',
  )
    .bind(slug)
    .first()
  ...
  const existing = await env.DB.prepare(
    'SELECT user_id FROM post_favorites WHERE user_id = ? AND post_id = ?',
  )
  ...
  if (existing) {
    await env.DB.prepare('DELETE FROM post_favorites WHERE user_id = ? AND post_id = ?')
      .bind(user.id, post.id)
      .run()
    await env.DB.prepare(
      'UPDATE posts SET favorite_count = CASE WHEN favorite_count > 0 THEN favorite_count - 1 ELSE 0 END WHERE id = ?',
    )
      .bind(post.id)
      .run()
    favorited = false
    ...
    await env.DB.prepare(
      'INSERT INTO post_favorites (user_id, post_id, created_at) VALUES (?, ?, ?)',
    )
      .bind(user.id, post.id, now)
      .run()
    await env.DB.prepare('UPDATE posts SET favorite_count = favorite_count + 1 WHERE id = ?')
      .bind(post.id)
      .run()
    favorited = true
  }
  ...
  return json(200, {
    ...
    heat: enriched.heat,
    favoritedByMe: enriched.favoritedByMe,
